...
export default function AdminPanel({ data, onSave, onReset }: AdminPanelProps) {
  ...
  function handleReset() {
    setConfirm({ title: 'Reset Semua Data', msg: 'PERINGATAN: Semua data akan dikembalikan ke pengaturan awal. Tindakan ini tidak dapat dibatalkan. Lanjutkan?', action: async () => {
      try {
        await onReset();
        setConfirm(null);
        ...
        setConfirm(null);
        const msg = err instanceof Error ? err.message : String(err);
        showToast(`Gagal reset: ${msg}`, 'error');
      }
    }});
  }
