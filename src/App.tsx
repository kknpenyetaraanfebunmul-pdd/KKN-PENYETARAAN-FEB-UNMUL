    ...
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
        ...
        >
          ...
          <div className="relative z-10 text-center px-6 max-w-4xl">
            ...
            <p className="text-sm sm:text-base text-[#2b1c3d] font-semibold max-w-xl mx-auto mb-6">
              {data.hero.desc}
            </p>
            <button
        ...
        >
          {data.struktur.map((item, i) => (
            <div key={i} className="text-center py-2">
              <h3 className="text-sm sm:text-lg font-extrabold text-[#2b1c3d] uppercase tracking-wide">
                {item.title}
              </h3>
              <p className="text-[10px] sm:text-xs font-semibold text-gray-400 uppercase tracking-wide mt-1">
                {item.subtitle}
              </p>
            </div>
          ))}
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {data.program.map((p) => (
            ...
            >
              ...
              <div className="text-xl text-[#2b1c3d] group-hover:translate-x-1 group-hover:text-[#7b5ea7] transition">
                →
              </div>
            </button>
          ))}
        ...
        >
          <div className="relative z-10 max-w-3xl mx-auto">
            <div className="text-[10px] sm:text-xs font-bold uppercase tracking-[3px] text-[#7b5ea7] mb-3">
              Mari Terhubung
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-2xl mx-auto">
              <a
                ...
                target="_blank"
                rel="noopener"
                className="flex items-center gap-3 p-4 rounded-2xl bg-white/85 backdrop-blur border border-white hover:-translate-y-1.5 hover:shadow-xl hover:border-[#7b5ea7] transition"
              >
              ...
              <div className="mb-8">
                ...
                <div className="flex flex-wrap justify-center items-center gap-6 sm:gap-12">
                  {data.partners.support.map((p, i) => (
                    <div
                      key={i}
                      className="h-14 sm:h-16 grayscale opacity-60 hover:grayscale-0 hover:opacity-100 hover:scale-110 hover:-translate-y-1 transition duration-300"
                      title={p.name}
                    >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-0.5 px-3 py-2 rounded-2xl min-w-[56px] transition-all ${
              activeSection === item.id
                ? 'bg-gradient-to-br from-[#7b5ea7]/20 to-[#7b5ea7]/10 text-[#7b5ea7]'
                : 'text-[#2b1c3d] hover:bg-[#7b5ea7]/10'
            }`}
          >
      ...
      {programModal && (
        ...
        >
          ...
          >
            <button
              onClick={() => setProgramModal(null)}
              className="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/90 shadow flex items-center justify-center text-[#2b1c3d] hover:bg-white hover:rotate-90 transition z-10"
            >
              ✕
            </button>
            ...
            <div className="p-8 sm:p-10">
              <div className="mb-6">
                <div className="text-[11px] font-extrabold uppercase tracking-widest text-[#7b5ea7] mb-3 flex items-center gap-2">
                  Deskripsi Program
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    {programModal.info.map((info, i) => (
                      <div key={i} className="p-3 rounded-xl bg-[#ece6f5]">
                        <div className="text-[10px] font-bold uppercase tracking-wider text-[#7b5ea7] mb-1">
                          {info.label}
                        </div>
      ...
      {lightboxIndex !== null && data.gallery[lightboxIndex] && (
        ...
        >
          <button
            onClick={() => setLightboxIndex(null)}
            className="absolute top-4 right-4 w-12 h-12 rounded-full bg-white/10 border border-white/15 text-white flex items-center justify-center text-xl hover:bg-white/20 hover:rotate-90 transition"
          >
            ✕
          </button>
          ...
          {data.gallery.length > 1 && (
            <>
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setLightboxIndex((lightboxIndex + 1) % data.gallery.length);
                }}
                className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 border border-white/15 text-white flex items-center justify-center text-2xl hover:bg-white/20 transition"
              >
                ›
              </button>
            </>
          ...
          <div className="max-w-full max-h-full flex flex-col items-center" onClick={(e) => e.stopPropagation()}>
            <div className="max-w-[min(1100px,92vw)] max-h-[80vh] rounded-2xl overflow-hidden shadow-2xl bg-black">
              {data.gallery[lightboxIndex].type === 'video' ? (
                <video
                  src={data.gallery[lightboxIndex].src}
                  controls
                  autoPlay
                  playsInline
                ...
                <img
                  src={data.gallery[lightboxIndex].src}
                  alt={data.gallery[lightboxIndex].title}
                  className="max-w-full max-h-[80vh] object-contain block"
                />
              )}
            </div>
            <div className="mt-5 text-center text-white max-w-xl px-3">
              <h4 className="text-lg font-bold uppercase tracking-wide mb-1.5">
                {data.gallery[lightboxIndex].title}
              </h4>
