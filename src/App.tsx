...
export default function App() {
  ...
  useEffect(() => {
    const unsub = subscribeToData((newData) => {
      ...
      if (!isAdminRef.current) showToast('Data diperbarui');
    });
  ...
  return (
    <>
      ...
      <ProgramModal />
      {toast && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-green-500 text-white px-6 py-3 rounded-xl font-semibold text-sm shadow-lg z-[10002] flex items-center gap-2">
          {'\u2714'} {toast}
        </div>
      )}
      {syncToast && (
        <div className="fixed bottom-20 left-1/2 -translate-x-1/2 bg-[#7b5ea7] text-white px-5 py-2.5 rounded-xl font-semibold text-sm shadow-lg z-[10002] flex items-center gap-2">
          {'\u{1F504}'} {syncToast}
        </div>
      )}
    </>
...
function LoginModal({ onClose, onLogin }: { onClose: () => void; onLogin: (passcode: string) => void }) {
  ...
  function handleSubmit() {
    if (passcode === ADMIN_PASSCODE) onLogin(passcode);
    else {
  ...
  return (
    <div className="fixed inset-0 bg-[rgba(20,12,30,0.6)] backdrop-blur-sm z-[10000] flex items-center justify-center p-5" onClick={onClose}>
      <div className={`bg-white rounded-3xl p-8 sm:p-10 w-full max-w-[380px] shadow-2xl text-center ${shake ? 'animate-[shake_0.4s_ease]' : ''}`} onClick={(e) => e.stopPropagation()}>
        <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#7b5ea7] to-[#2b1c3d] flex items-center justify-center mx-auto mb-5 text-3xl shadow-lg">{'\u{1F512}'}</div>
        ...
        <input ref={inputRef} type="password" maxLength={6} inputMode="numeric" pattern="[0-9]*" placeholder="000000" value={passcode} onChange={(e) => { setPasscode(e.target.value.replace(/[^0-9]/g, '')); setError(''); }} onKeyDown={(e) => { if (e.key === 'Enter') handleSubmit(); }} className={`w-full px-4 py-3.5 rounded-xl border-2 text-lg font-semibold text-center tracking-[4px] text-[#2b1c3d] outline-none transition ${error ? 'border-red-500' : 'border-[#e0d9e8] focus:border-[#7b5ea7]'}`} />
        <div className="text-red-500 text-xs font-semibold mb-4 min-h-[18px]">{error}</div>
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-3 rounded-xl bg-gray-100 text-gray-600 font-semibold text-sm">Batal</button>
          <button onClick={handleSubmit} className="flex-1 py-3 rounded-xl bg-gradient-to-br from-[#7b5ea7] to-[#2b1c3d] text-white font-semibold text-sm">Masuk</button>
        </div>
      </div>
    </div>
  );
...
function SiteView({ data, loading, syncing, onFooterClick, showLogin, onLoginClose, onLogin }: { data: SiteData; loading: boolean; syncing: boolean; onFooterClick: () => void; showLogin: boolean; onLoginClose: () => void; onLogin: (passcode: string) => void }) {
  return (
    <div className="min-h-screen bg-[#f7f5fb] text-[#333] font-[Poppins,sans-serif] overflow-x-hidden">
      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-10">
        <header className="flex items-center justify-between py-4 sm:py-5 relative z-100">
          ...
          <nav className="hidden md:flex gap-1 text-xs font-semibold uppercase tracking-wider">
            {['Home', 'Struktur', 'Program Kerja', 'Gallery', 'Contact'].map((label, i) => (
              <a key={label} href={`#${['home', 'struktur', 'program', 'gallery', 'contact'][i]}`} className="px-4 py-2.5 rounded-full text-[#2b1c3d] hover:text-[#7b5ea7] transition">{label}</a>
            ))}
          </nav>
        </header>

        <section className="relative flex items-center justify-center text-center overflow-hidden sm:rounded-[30px] mt-2 sm:mt-5 min-h-[60vh] sm:min-h-[500px] py-12 sm:py-20" id="home">
          <div className="absolute inset-0 z-0 overflow-hidden">
            {data.hero.slideshow.map((url, i) => (
              <div key={i} className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url('${escapeHtml(url)}')`, animation: `slideCarousel 12s infinite`, animationDelay: `${i * 4}s` }} />
            ))}
          ...
          <div className="relative z-2 max-w-[850px] mx-auto px-2">
            ...
            <a href="#struktur" className="inline-flex items-center gap-2.5 bg-[#7b5ea7] text-white px-6 sm:px-9 py-3 sm:py-4 rounded-full text-sm font-semibold hover:bg-[#2b1c3d] transition min-h-[44px]">View KKN</a>
          </div>
        </section>

        <section className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 sm:gap-5 bg-white rounded-[20px] p-6 sm:p-10 -mt-5 shadow-[0_10px_30px_rgba(0,0,0,0.05)] relative z-10 text-center" id="struktur">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
          {data.program.map((p) => (
            <div key={p.id} role="button" tabIndex={0} onClick={() => openProgramModal(p)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openProgramModal(p); } }} className="bg-[#ece6f5] p-6 sm:p-8 rounded-[20px] flex flex-col cursor-pointer transition hover:-translate-y-1 hover:shadow-lg select-none">
              <div className="w-11 h-11 bg-white rounded-full flex items-center justify-center mb-4 text-xl">{escapeHtml(p.icon)}</div>
              ...
              <div className="text-xl text-[#2b1c3d]">{'\u2192'}</div>
            </div>
          ))}
        </section>
        ...
        <section className="mt-16 sm:mt-24 px-5 sm:px-14 py-12 sm:py-20 bg-gradient-to-br from-[#ece6f5] via-[#ded0f0] to-[#ece6f5] rounded-[24px] sm:rounded-[40px] relative overflow-hidden" id="contact">
          <div className="relative z-1 text-center max-w-[900px] mx-auto">
            ...
            <p className="text-sm text-gray-500 max-w-[560px] mx-auto mb-10 sm:mb-12 leading-relaxed">Kami terbuka untuk kolaborasi, pertanyaan, atau sekadar berbagi cerita seputar KKN Penyetaraan. Jangan ragu untuk menghubungi kami melalui platform di bawah ini.</p>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 max-w-[800px] mx-auto">
              <a href={data.contact.instagramUrl} target="_blank" rel="noopener" className="flex items-center gap-3.5 p-4 sm:p-6 bg-white/85 backdrop-blur rounded-[20px] border border-white/90 shadow-sm hover:-translate-y-1.5 hover:shadow-lg transition">
              ...
              <a href={data.contact.tiktokUrl} target="_blank" rel="noopener" className="flex items-center gap-3.5 p-4 sm:p-6 bg-white/85 backdrop-blur rounded-[20px] border border-white/90 shadow-sm hover:-translate-y-1.5 hover:shadow-lg transition">
              ...
              <a href={`mailto:${data.contact.email}`} className="flex items-center gap-3.5 p-4 sm:p-6 bg-white/85 backdrop-blur rounded-[20px] border border-white/90 shadow-sm hover:-translate-y-1.5 hover:shadow-lg transition">
            ...
            <div className="mt-12 sm:mt-14 p-7 sm:p-12 bg-white rounded-[24px] sm:rounded-[32px] shadow-md relative overflow-hidden">
              <div className="mb-8 sm:mb-12">
                <div className="text-xs font-extrabold uppercase tracking-[3px] text-[#7b5ea7] mb-6 sm:mb-9">Support By</div>
                <div className="flex flex-wrap items-center justify-center gap-6 sm:gap-12">
                  {data.partners.support.map((p) => (
                    <div key={p.id} className="flex items-center justify-center h-12 sm:h-18 max-w-[160px] transition hover:scale-110 cursor-pointer" title={escapeHtml(p.name)}>
              ...
              <div>
                <div className="text-xs font-extrabold uppercase tracking-[3px] text-[#7b5ea7] mb-6 sm:mb-9">Sponsor By</div>
                <div className="flex flex-wrap items-center justify-center gap-6 sm:gap-12">
            ...
            <div className="mt-8 sm:mt-12 pt-6 sm:pt-7 border-t border-[rgba(123,94,167,0.15)] flex flex-wrap gap-3 justify-between items-center text-xs text-gray-400">
              <div onClick={onFooterClick} className="cursor-pointer select-none">
...
function GallerySection({ items }: { items: SiteData['gallery'] }) {
  ...
  return (
    <>
      <style>{`
        .gallery-scroll { display: grid; grid-auto-flow: column dense; grid-template-rows: repeat(2, 1fr); grid-auto-columns: 180px; height: 500px; gap: 14px; overflow-x: auto; overflow-y: hidden; padding: 8px 4px; scrollbar-width: none; }
        ...
        .gallery-item.is-video::before { content: '\\25B6 VIDEO'; position: absolute; top: 12px; left: 12px; padding: 4px 10px; background: rgba(43,28,61,0.85); color: white; font-size: 9px; font-weight: 700; border-radius: 20px; z-index: 3; }
        .gallery-caption { position: absolute; bottom: 0; left: 0; right: 0; padding: 14px 16px; color: white; z-index: 4; background: linear-gradient(to top, rgba(43,28,61,0.85), transparent); }
      `}</style>
          ...
          {items.map((item, i) => {
            ...
            return (
              <div key={item.id} className={`gallery-item ${sizeClass} ${item.type === 'video' ? 'is-video' : ''}`} onClick={() => setLightbox(i)}>
                {item.type === 'video' ? (
                  <video autoPlay muted loop playsInline><source src={escapeHtml(item.src)} type="video/mp4" /></video>
                ) : (
      ...
      {lightbox !== null && (
        <div className="fixed inset-0 bg-[rgba(20,12,30,0.92)] backdrop-blur-md z-[9999] flex items-center justify-center p-4 sm:p-16" onClick={() => setLightbox(null)}>
          <button className="absolute top-4 right-4 w-12 h-12 rounded-full bg-white/10 border border-white/15 text-white text-xl flex items-center justify-center z-100" onClick={() => setLightbox(null)}>{'\u2715'}</button>
          <button className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 border border-white/15 text-white text-lg flex items-center justify-center z-100" onClick={(e) => { e.stopPropagation(); setLightbox((p) => (p === null ? null : (p - 1 + items.length) % items.length)); }}>{'\u2039'}</button>
          <button className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 border border-white/15 text-white text-lg flex items-center justify-center z-100" onClick={(e) => { e.stopPropagation(); setLightbox((p) => (p === null ? null : (p + 1) % items.length)); }}>{'\u203A'}</button>
  ...
  return (
    <div className="fixed inset-0 bg-[rgba(20,12,30,0.7)] backdrop-blur-md z-[9998] flex items-center justify-center p-4 sm:p-10" onClick={() => { programModalState = { open: false, program: null }; programModalListeners.forEach((fn) => fn(programModalState)); }}>
      <div className="relative w-full max-w-[620px] max-h-[90vh] overflow-y-auto bg-white rounded-3xl shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <button className="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/90 text-[#2b1c3d] text-lg flex items-center justify-center z-10" onClick={() => { programModalState = { open: false, program: null }; programModalListeners.forEach((fn) => fn(programModalState)); }}>{'\u2715'}</button>
        <div className="relative p-7 sm:p-10 bg-gradient-to-br from-[#ece6f5] to-[#ded0f0] rounded-t-3xl overflow-hidden">
          <div className="relative z-1">
            <div className="bg-white rounded-full flex items-center justify-center text-4xl mb-5 shadow-md" style={{ width: 72, height: 72 }}>{escapeHtml(p.icon)}</div>
        ...
        <div className="p-6 sm:p-10">
          <div className="mb-6">
            <div className="text-xs font-extrabold uppercase tracking-wide text-[#7b5ea7] mb-3">Deskripsi Program</div>
            <p className="text-sm text-gray-600 leading-relaxed">{escapeHtml(p.description)}</p>
          </div>
          <div className="mb-6">
            <div className="text-xs font-extrabold uppercase tracking-wide text-[#7b5ea7] mb-3">Kegiatan Utama</div>
            <div className="flex flex-col gap-2.5">
              {activities.map((a, i) => (
                <div key={i} className="flex items-start gap-3 p-3 bg-[#ece6f5] rounded-xl">
                  <div className="bg-white rounded-full flex items-center justify-center text-xs text-[#7b5ea7] font-bold shrink-0" style={{ width: 22, height: 22 }}>{i + 1}</div>
          ...
          <div>
            <div className="text-xs font-extrabold uppercase tracking-wide text-[#7b5ea7] mb-3">Informasi</div>
