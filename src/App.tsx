...
export default function App() {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        ...
        // silent for polling — error already shown on initial load
      }
    }, 30000);
  ...
  const handleSave = useCallback(async (newData: SiteData) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      throw new Error(`Gagal menyimpan ke server: ${msg}`);
    }
  }, []);
  ...
  const handleReset = useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      throw new Error(`Gagal reset server: ${msg}`);
    }
  }, []);
  ...
  return (
    <>
      ...
      {errorToast && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 bg-red-500 text-white px-6 py-3 rounded-xl font-semibold text-sm shadow-lg z-[10003] flex items-center gap-2 max-w-[90vw] text-center animate-[fadeInUp_0.3s_ease]">
          {'\u26A0'} {errorToast}
        </div>
      )}
    </>
